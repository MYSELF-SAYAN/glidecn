...
export function getTemplatesDir(): string {
  ...
  throw new Error(
    `Could not find Morphy templates directory. Checked:\n${candidates.map((c) => `  - ${c}`).join('\n')}`,
  );
}
...
/**
 * Detect language of an existing Morphy installation.
 */
export function detectProjectLanguage(morphyDir: string): 'ts' | 'js' {
  if (fs.existsSync(path.join(morphyDir, 'page.jsx')) || fs.existsSync(path.join(morphyDir, 'index.js'))) {
    return 'js';
  }
  return 'ts';
}
...
/**
 * Copy core Morphy files (everything except transitions and adapters).
 */
...
/**
 * Generate the main index.ts / index.js barrel export for the installed Morphy.
 */
export async function generateBarrelExport(
  ...
  const lines = [
    '/* ==========================================================================',
    ' * Morphy — Public API',
    ' * Auto-generated by morphy CLI. Feel free to customize!',
    ' * ========================================================================== */',
    '',
    '// Core components',
    "export { MorphyProvider } from './core/provider';",
    "export { TransitionManager as Morphy, TransitionManager } from './core/transition-manager';",
...
function getAdapterExportLines(adapter: string): string[] {
  switch (adapter) {
    case 'next-app':
      return [
        "export { MorphyNextApp, FrozenRouter } from './adapters/next-app';",
      ];
    case 'next-pages':
      return [
        "export { MorphyNextPages } from './adapters/next-pages';",
      ];
    case 'react-router':
      return [
        "export { MorphyReactRouter } from './adapters/react-router';",
      ];
    case 'universal':
      return [
        "export { MorphyUniversal } from './adapters/universal';",
      ];
    default:
      return [
        "export { MorphyUniversal } from './adapters/universal';",
